...
app.get("/auth/google/callback", passport.authenticate("google", { failureRedirect: `${FRONTEND_URL}/signin?error=google_failed` }), (req, res) => {
    const user = req.user;
...
app.post("/signup", async (req, res) => {
    ...
    if (!parsedData.success) {
        const errors = parsedData.error.issues.map((e) => e.message).join(", ");
...
app.post("/shapes/:roomId", middleware, async (req, res) => {
    ...
    if (!Array.isArray(shapes))
        return res.status(400).json({ message: "Expected array of shapes" });
    try {
        await Promise.all(shapes.map((shape) => prisma.shape.upsert({
            where: { id: shape.id },
